...
export function getBendpoints(a, b, directions) {
  ...
  var startDirection = directions.split(':')[0],
      endDirection = directions.split(':')[1];
  ...
  if (directions === 'v:v') {
    ...
    if (endDirection === 'v') {
      return [
        { x: a.x, y: max(a.y + MIN_SEGMENT_LENGTH, b.y) },
        { x: b.x, y: max(a.y + MIN_SEGMENT_LENGTH, b.y) }
      ];
    } else if (endDirection === 'h') {
      xmid = round((b.x - a.x) / 2 + a.x);

      return [
        { x: a.x, y: max(a.y + MIN_SEGMENT_LENGTH, b.y) },
        { x: xmid, y: max(a.y + MIN_SEGMENT_LENGTH, b.y) },
        { x: xmid, y: b.y }
      ];
    }
    ...
    throw new Error(
      ...
      'directions must be specified as {a direction}:{b direction} (direction as in h|v|t|r|b|l)');
  }
}
...
export function connectRectangles(source, target, start, end, hints) {
  ...
  if (!directions) {
    directions = preferredLayout;
  }

  var directionsStart = directions.split(':')[0],
      directionsEnd = directions.split(':')[1];
  ...
  if (directions === 'h:h') {
    ...
    switch (directionsStart) {
    case 't':
      start = { original: start, x: start.x, y: source.y };
      break;
    case 'r':
      start = { original: start, x: start.x + start.width, y: source.y };
      break;
    case 'b':
      start = { original: start, x: start.x, y: source.y + source.height };
      break;
    ...
    case 't':
      ...
      break;
    case 'r':
      end = { original: end, x: end.x + end.width, y: target.y };
      break;
    case 'b':
      end = { original: end, x: end.x, y: target.y + target.height };
      break;
...
function getDirections(orientation, defaultLayout) {
  ...
  if (/t|r|b|l/.test(defaultLayout)) {

    // default layout is already more specific
